...
exports.createFile = async (session, {fileName, fileDescription, fileTags, projectId, upload}) => {
  ...
  const file = {
    ...
    fileName,
    fileOriginalName: upload.originalname,
...
async function createActivityPush(userId, file, activityDescription, upload) {
  ...
  try {
    const params = {
      ...
      Key: {fileId: file.fileId, projectId: file.projectId},
      UpdateExpression:
        'set #fileActivities = list_append(:fileActivities, #fileActivities), #fileOriginalName = :fileOriginalName, #fileVersion = :fileVersion, #fileUpdateTime = :fileUpdateTime',
      ExpressionAttributeNames: {
        '#fileActivities': 'fileActivities',
